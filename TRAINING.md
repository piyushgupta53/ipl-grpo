...
```bash
PYTHONPATH=src python3 -m ipl_reasoner.cli run-sft-warmup --dry-run --dataset-variant first_pass_all
```
...
2. Run the SFT warmup from the prompt/completion dataset, starting with `first_pass_all`.
...
- The default rerun dataset is `first_pass_all`, which replaces the earlier draft-backed warmup labels.
